...
from deepspeed.runtime.utils import prefetchtable, memory_manager, active_tensor_window, GPU_MEMORY_CACHE, CPU_MEMORY_CACHE
...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def _partitioned_params_swap_out(self, i):
        ...
        if not prefetchtable.get_warmup():
            param_16 = self.param_dict[self.fp32_partitioned_groups_flat_new[i].ds_id]
            swap_fp16_params = []
            swap_fp32_params = []
            swap_fp32_params.append(self.fp32_partitioned_groups_flat_new[i].data)
            ...
            if self.offload_optimizer:
                ...
                if self.is_gradient_accumulation_boundary:
                    ...
                    if self._swappable_optimizer_subgroup(i):
                        ...
                        if not prefetchtable.get_warmup():
                            fp32_grad_tensor = self.fp32_partitioned_groups_flat_new[i].grad.narrow(
                                0, dest_offset, grad_partition.numel())
                            fp32_grad_tensor.copy_(grad_partition.float())
